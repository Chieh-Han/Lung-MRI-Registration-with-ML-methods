...
def value_at_location(
    x, single_vol=False, single_pts=False, force_post_absolute_val=True
):
    """
...
def transform(
    ...
    # test single
    return ne.utils.interpn(
        vol, loc, interp_method=interp_method, fill_value=fill_value
    )


def batch_transform(
    vol, loc_shift, batch_size=None, interp_method="linear", fill_value=None
):
    ...
    assert ndim in range(1, 4), f"Dimension {ndim} can only be in [1, 2, 3]"
    ...
    if batch_size is None:
        ...
        ), "batch_transform: provide batch_size or valid Tensor shape"
    else:
    ...
    assert (
        len(loc_shift.shape) == ndim + 3
    ), "vol dim {} and loc dim {} are not appropriate".format(
        ndim + 2, len(loc_shift.shape)
    )
    ...
    loc_reshape = K.permute_dimensions(
        loc_shift, list(range(1, ndim + 2)) + [0] + [ndim + 2]
    )
    loc_reshape_shape = (
        list(vol.shape[1 : ndim + 1]) + [BC] + [loc_shift.shape[ndim + 2]]
    )
    loc_reshape = K.reshape(loc_reshape, loc_reshape_shape)

    # transform (output is [*vol_shape, C*B])
    vol_trf = transform(
        vol_reshape, loc_reshape, interp_method=interp_method, fill_value=fill_value
    )
...
def compose(transforms, interp_method="linear", shift_center=True, shape=None):
    ...
    for next in reversed(transforms):
        ...
        # Matrix on left, dense warp on right: matrix-vector product.
            ...
            curr = affine_to_dense_shift(
                next, shape=curr.shape[:-1], shift_center=shift_center, warp_right=curr
            )
...
def keras_transform(img, trf, interp_method="linear", rescale=None):
    ...
    trf_scaled = (
        trf_input if rescale is None else layers.RescaleTransform(rescale)(trf_input)
    )
    y_img = layers.SpatialTransformer(interp_method=interp_method)(
        [img_input, trf_scaled]
    )
...
def validate_affine_shape(shape):
    ...
    if rows not in (ndim, ndim + 1):
        raise ValueError(
            f"{ndim}D affine matrix must have {ndim} or {ndim + 1} rows, got {rows}."
        )
...
def affine_to_dense_shift(matrix, shape, shift_center=True, warp_right=None):
    ...
    if warp_right is not None:
        ...
        flat_shape = tf.concat(
            (tf.shape(warp_right)[: -1 - ndims], (-1, ndims)), axis=0
        )
    ...
    out = (
        matrix[..., :ndims, :-1] @ out + matrix[..., :ndims, -1:]
    )  # ... x N x nb_voxels
...
def angles_to_rotation_matrix(ang, deg=True, ndims=3):
    ...
    if shape[-1] > num_ang:
        raise ValueError(
            f"Number of angles exceeds value {num_ang} expected for dimensionality."
        )
...
def params_to_affine_matrix(par, deg=True, shift_scale=False, last_row=False, ndims=3):
    """
    ...
    if shape[-1] > num_par:
        raise ValueError(
            f"Number of params exceeds value {num_par} expected for dimensionality."
        )
    ...
    for i in (2, 3, 4):
        ...
        default = 1.0 if i == 3 and not shift_scale else 0.0
    ...
    mat_scale = tf.linalg.diag(scale + 1.0 if shift_scale else scale)
